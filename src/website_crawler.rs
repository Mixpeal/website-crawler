...
/// ```rust
/// use spider::website::WebsiteCrawler;
/// let mut website = WebsiteCrawler::new("http://example.com");
/// website.crawl();
/// // `WebsiteCrawler` will be filled with `Pages` when crawled. To get them, just use
...
pub struct WebsiteCrawler {
    ...
    /// callback when a link is found.
    pub on_link_found: Option<Arc<Mutex<Box<dyn Fn(&CaseInsensitiveString) + Send>>>>,
...
impl fmt::Debug for WebsiteCrawler {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("WebsiteCrawler")
            .field("configuration", &self.configuration)
            .field("links_visited", &self.links_visited)
            .field("pages", &self.pages)
            // Don't output `on_link_find_callback` and `on_link_found`
            .field("robot_file_parser", &self.robot_file_parser)
            .field("domain", &self.domain)
            .field("domain_parsed", &self.domain_parsed)
            .finish()
    }
}

impl WebsiteCrawler {
    /// Initialize WebsiteCrawler object with a start link to crawl.
    ...
    async fn crawl_establish(
            ...
            .is_allowed_default(&self.get_base_link(), &self.configuration.get_blacklist())
        {
            ...
            let url_backup: CaseInsensitiveString = page.get_url().into();
            if let Some(callback) = &self.on_link_found {
    ...
    async fn crawl_concurrent(&mut self, client: Client, handle: Option<Arc<AtomicI8>>) {
        ...
        if selectors.is_some() {
            ...
            let on_link_found = self.on_link_found.clone();
                ...
                loop {
                    match stream.next().await {
                        Some(link) => {
                            ...
                            let on_link_found = on_link_found.clone();
                            ...
                            set.spawn_on(
                                async move {
                                    ...
                                    if let Some(callback) = on_link_found {
                                        let callback = callback.lock().unwrap();
                                        callback(&link_clone);
                                    }
        ...
        if selectors.is_some() {
            ...
            loop {
                for link in links.iter() {
                    ...
                    let link_backup: CaseInsensitiveString = link.clone();
                    ...
                    if let Some(ref callback) = on_link_found {
                        let callback = callback.lock().unwrap();
                        callback(&link_backup);
                    }
...
async fn crawl() {
    ...
    let mut website: WebsiteCrawler = WebsiteCrawler::new(&url);
    website.crawl().await;
    assert!(
        website
            .links_visited
...
#[tokio::test]
async fn scrape() {
    let mut website: WebsiteCrawler = WebsiteCrawler::new("https://choosealicense.com");
...
async fn crawl_invalid() {
    ...
    let mut website: WebsiteCrawler = WebsiteCrawler::new(domain);
    website.crawl().await;
...
async fn test_respect_robots_txt() {
    let mut website: WebsiteCrawler = WebsiteCrawler::new("https://stackoverflow.com");
    ...
    let mut website_second: WebsiteCrawler = WebsiteCrawler::new("https://www.mongodb.com");
    ...
    let mut website_third: WebsiteCrawler = WebsiteCrawler::new("https://www.mongodb.com");
...
async fn test_with_configuration() {
    let mut website = WebsiteCrawler::new("https://choosealicense.com");
...
async fn test_crawl_glob() {
    let mut website: WebsiteCrawler =
        WebsiteCrawler::new("https://choosealicense.com/licenses/{mit,apache-2.0,mpl-2.0}/");
    website.crawl().await;
...
async fn test_link_duplicates() {
    ...
    let mut website: WebsiteCrawler = WebsiteCrawler::new("http://0.0.0.0:8000");
    website.crawl().await;
